...
export class MeetWidget extends Component {
  ...
  render() {
    ...
    if (displayName) {
      ...
      // Is the call active?
      const meeting = this.props.sdkAdapter.meetingsAdapter.datasource.meetings.getMeetingByType('id', call.id);

      if (meeting && (meeting.meetingFiniteStateMachine.is('JOINED') || widgetMeet.status.hasInitiatedCall)) {
        return (
          <div className={classNames(styles.meetWidgetContainer, 'meet-widget-container')}>
            <ActiveMeeting adapter={this.props.sdkAdapter} meetingId={meeting.id} />
